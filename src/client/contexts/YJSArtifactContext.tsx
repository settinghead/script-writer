...
export const YJSArtifactProvider: React.FC<YJSArtifactProviderProps> = ({
    ...
    const mergedData = React.useMemo(() => {
        ...
        if (yjsData && typeof yjsData === 'object' && !Array.isArray(yjsData)) {
            ...
            try {
                ...
                console.warn('Failed to parse YJS data as JSON:', yjsData);
                base = {};
            }
        }
        ...
        Object.entries(optimisticUpdatesRef.current).forEach(([path, value]) => {
            try {
                ...
                console.error(`Failed to set path ${path}:`, e);
            }
        });

        return merged;
    }, [yjsData, pendingUpdates]);
    ...
    const setField = useCallback((path: string, value: any) => {
        ...
        // Use a timeout to defer state updates and prevent render-time updates
        setTimeout(() => {
            try {
                ...
                if (isCollaborative && yjsData) {
                    // For YJS, we need to update the root-level field
                    const rootKey = path.split('.')[0];
                    const currentRootValue = yjsData?.[rootKey] || {};
                ...
                console.error(`Error setting field ${path}:`, error);
            }
        }, 0);
    }, [isCollaborative, yjsUpdateField, yjsData]);
                ...
                if (isCollaborative) {
                    ...
                    Object.entries(updates).forEach(([path, value]) => {
                        ...
                        if (!rootUpdates[rootKey]) {
                            rootUpdates[rootKey] = yjsData?.[rootKey] || {};
                        }

                        if (path.includes('.')) {
                            rootUpdates[rootKey] = setValueByPath(rootUpdates[rootKey], path, value);
                        } else {
                            rootUpdates[rootKey] = value;
                        }
                    });

                    yjsUpdateFields(rootUpdates);
                }
            } catch (error) {
                console.error('Error updating multiple fields:', error);
            }
        }, 0);
    }, [isCollaborative, yjsUpdateFields, yjsData]);
