...
export function createCachedStreamObjectMock() {
    return vi.fn().mockImplementation(async (options: {
        ...
        if (cachedResponse) {
            ...
            if (prompt.includes('chronicles') || prompt.includes('template: chronicles') || prompt.includes('templateName: chronicles') || (prompt.includes('Chronological Outline') && prompt.includes('创作一个**时间顺序故事描述')) || (prompt.includes('时间顺序故事描述') && (prompt.includes('创作一个**时间顺序故事描述') || prompt.includes('请创作一个**时间顺序故事描述'))) || prompt.includes('时间顺序') || prompt.includes('timeline') || prompt.includes('stages')) {
