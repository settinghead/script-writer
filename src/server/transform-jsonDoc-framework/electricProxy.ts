...
export function createElectricProxyRoutes(authDB: AuthDatabase, jsonDocRepo: JsonDocRepository) {
    ...
    router.get('/v1/shape', authMiddleware.authenticate, async (req: Request, res: Response): Promise<void> => {
        ...
        try {
            ...
            // 2. For YJS tables, validate jsonDoc access before proceeding
            if (table === 'jsonDoc_yjs_documents' || table === 'jsonDoc_yjs_awareness') {
                const existingWhere = url.searchParams.get('where');
                if (!existingWhere) {
                    res.status(400).json({ error: `YJS table ${table} requires jsonDoc_id filter` });
                    return;
                }

                // Extract jsonDoc_id from the where clause
                const jsonDocIdMatch = existingWhere.match(/jsonDoc_id\s*=\s*'([^']+)'/);
                if (!jsonDocIdMatch) {
                    res.status(400).json({ error: `YJS table ${table} requires valid jsonDoc_id filter` });
                    return;
                }

                const jsonDocId = jsonDocIdMatch[1];

                // Check if user has access to this jsonDoc via project membership
                try {
                    const hasAccess = await jsonDocRepo.userHasJsonDocAccess(user.id, jsonDocId);
                    if (!hasAccess) {
                        res.status(403).json({ error: 'Access denied to jsonDoc' });
                        return;
                    }
                } catch (error) {
                    console.error('[Electric Proxy] Error checking jsonDoc access:', error);
                    res.status(500).json({ error: 'Failed to verify jsonDoc access' });
                    return;
                }
            }
            ...
            switch (table) {
                ...
                case 'jsonDoc_yjs_awareness':
                    ...
                    if (existingWhere) {
                        // Keep the existing where clause (which should include jsonDoc_id filter)
                        finalWhereClause = existingWhere;
                    } else {
