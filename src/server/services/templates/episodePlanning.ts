...
export const episodePlanningTemplate: LLMTemplate = {
    ...
    promptTemplate: `你是一位专门从事抖音短剧内容的资深编剧。你的任务是基于时间顺序大纲，创建适合短视频平台的剧集框架，重点关注节奏紧凑、起承转合的完整戏剧结构。
...
**重要原则：不要完全按照原始灵感的节奏来安排，而是要重新优化剧集分配：**

1. **避免过早结束** - 不要让主要冲突在前2/3就解决完毕
2. **避免过度拖沓** - 不要为了凑集数而添加无关紧要的情节
3. **黄金分割法** - 重要转折点应该在总集数的60-70%处出现
4. **结局预留** - 最后20-25%的集数应该专注于高潮和结局
...
- episodeGroups: 分集阶段数组，每组包含：
  ...
  - dramaticFunction: 该阶段在整体戏剧结构中的功能（起/承/转/合）
  - keyEvents: 关键事件列表
  - hooks: 悬念钩子列表
