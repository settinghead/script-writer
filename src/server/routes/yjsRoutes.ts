...
async function saveUpdate({ jsondoc_id, update }: Update, userId?: string) {
    ...
    // Each update should be stored as a separate row so we can apply them in sequence
...
router.put('/update', async (req: any, res: any) => {
    ...
    try {
        ...
        try {
            ...
            // This is a more thorough validation without applying it to a document
            const Y = await import('yjs');
            try {
                // Create a temporary document just for validation
                const tempDoc = new Y.Doc();
