...
class BrainstormModule(dspy.Module):
    ...
    def forward(self, request: BrainstormRequest) -> List[StoryIdea]:
        """Generate story ideas based on the request"""
        
        # Build the requirements section
        requirements_text = request.requirements_section if request.requirements_section else ""
...
class OptimizedBrainstormModule(dspy.Module):
    ...
    def forward(self, request: BrainstormRequest) -> List[StoryIdea]:
        ...
        genre_analysis = self.analyze_genre(genre=request.genre)
...
要求：
- 生成恰好4个完整的故事情节梗概灵感
- 题材必须新颖创新，深度理解{request.genre}类型的核心特点，避免陈旧套路
- 每个创意包含一个吸引人的标题（3-7个字符）和完整故事梗概（50-80字）
- 故事结构：起承转合完整，主角鲜明，冲突强烈，发展合理，结局满意
- 高度适配{request.platform}平台特点和用户偏好
- 制作可行性：考虑拍摄成本、场景复杂度、演员要求等实际因素
- 题材一致性：严格符合{request.genre}类型的核心要素和观众期待
- 吸引度：具备强烈的情感共鸣和观看动机
"""
