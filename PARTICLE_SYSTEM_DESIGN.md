...
A semantic indexing and @mention system for the Transform Artifact Framework, enabling intelligent context construction and enhanced AI agent interactions through particle-based content retrieval.

## Overview

The Particle System provides semantic indexing of artifact content fragments (particles) with pgvector-based similarity search, enabling powerful @mention functionality in chat interfaces and intelligent context construction for AI agents.
...
- **Lineage-Aware** - Only active (leaf) artifacts generate @mention-able particles
...
RETURNS TRIGGER AS $$
BEGIN
    -- Notify the application about artifact changes
    PERFORM pg_notify('artifact_changed', 
        json_build_object(
            'artifact_id', COALESCE(NEW.id, OLD.id),
...
-- Attach trigger to artifacts table
CREATE TRIGGER artifact_change_trigger
    AFTER INSERT OR UPDATE OR DELETE ON artifacts
    FOR EACH ROW EXECUTE FUNCTION notify_artifact_change();
```
...
class ParticleEventBus extends EventEmitter {
    ...
    private async setupDatabaseListener() {
        ...
        await this.dbConnection.listen('artifact_changed', (payload: string) => {
            const data = JSON.parse(payload);
            this.handleArtifactChange(data);
        });
    }
    
    private async handleArtifactChange(data: any) {
        const { artifact_id, project_id, operation } = data;
        
        // Debounce rapid changes
        this.debouncedParticleUpdate(artifact_id, project_id, operation);
    }
    ...
    private debouncedParticleUpdate = debounce(async (
        ...
        try {
            if (operation === 'DELETE') {
                await this.particleService.deleteParticlesByArtifact(artifactId);
            } else {
                await this.particleService.updateParticlesForArtifact(artifactId, projectId);
            }
            
            // Emit event for any listeners
            this.emit('particles_updated', { artifactId, projectId, operation });
        } catch (error) {
...
CREATE INDEX idx_particles_artifact ON particles(artifact_id);
...
export class ParticleService {
    ...
    async updateParticlesForArtifact(artifactId: string, projectId: string): Promise<void> {
        const artifact = await this.getArtifact(artifactId);
        if (!artifact) return;
        
        // Check if artifact is active (leaf in lineage)
        const isActive = await this.isArtifactActive(artifactId);
        
        // Extract particles from artifact
        const particles = await this.particleExtractor.extractParticles(artifact);
    ...
    private async isArtifactActive(artifactId: string): Promise<boolean> {
        // Check if artifact is a leaf in the lineage (no dependents)
...
```typescript
// Extract particles from artifacts based on schema type
export class ParticleExtractor {
    ...
    async extractParticles(artifact: Artifact): Promise<ParticleData[]> {
        const extractor = this.extractors.get(artifact.schema_type);
        if (!extractor) return [];
        
        return await extractor.call(this, artifact);
    }
    
    private async extractBrainstormParticles(artifact: Artifact): Promise<ParticleData[]> {
    ...
    private async extractOutlineParticles(artifact: Artifact): Promise<ParticleData[]> {
        const data = artifact.data;
        const particles: ParticleData[] = [];
        ...
        if (data.selling_points && Array.isArray(data.selling_points)) {
            for (let i = 0; i < data.selling_points.length; i++) {
                ...
                particles.push({
                    id: `${artifact.id}_selling_point_${i}`,
    ...
    private async extractChroniclesParticles(artifact: Artifact): Promise<ParticleData[]> {
        const data = artifact.data;
        const particles: ParticleData[] = [];
...
export async function initializeParticleSystem(db: Kysely<DB>) {
    ...
    // Initial particle extraction for existing artifacts
...
1. **Real-time Synchronization**: PostgreSQL triggers ensure particles update immediately when artifacts change
...
5. **Lineage-Aware**: Only active (leaf) artifacts generate @mention-able particles
6. **Framework Integration**: Seamlessly integrates with existing Transform Artifact Framework
